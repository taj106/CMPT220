...
public class ATM {
    ...
    public static double loadBalance() {
        try (Scanner scanner = new Scanner(new File("atm_balance.txt"))) {
            if (scanner.hasNextLine()) {
                //read the line, remove necessary characters (like '$' and commas)
                String balanceStr = scanner.nextLine().replaceAll("[^0-9.-]", "");
                return Double.parseDouble(balanceStr);  //string into a double
            }
    ...
    public void deposit(double amount) {
        if (amount > 0) {
            ...
            saveBalance();
            System.out.println("Deposited: $" + formatAmount(amount));
        } else {
    ...
    public void withdraw(double amount) {
        if (amount > 0 && amount <= balance) {
            ...
            saveBalance();
            System.out.println("Withdrew: $" + formatAmount(amount));
        } else {
    ...
    public void checkBalance() {
        System.out.println("Current balance: $" + formatAmount(balance));  //display balance
    }

    //format the amount to show as $0.00
    ...
    public static void main(String[] args) {
        ...
        //load balance from the file and create an ATM instance
